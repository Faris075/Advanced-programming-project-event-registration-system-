 ...
 *
 * OWNER: Faris
 * USED BY: Mohamed Ahmed (RegistrationService – core booking logic)
 *          Islam (AdminRegistrationService – management + export)
 *
 * IMPORTANT: The pessimistic-lock query (findByIdWithLock) MUST be called
 * from within a @Transactional method to avoid "no transaction" exceptions.
 */
public interface RegistrationRepository extends JpaRepository<Registration, Long> {
    ...
    /**
     * Returns waitlisted registrations for an event in promotion order.
     * The record at position 1 is the next attendee to be promoted.
     *
     * NOTE: Use the fully-qualified enum reference in JPQL — string literals like 'WAITLISTED'
     * are not valid JPQL and will cause a query parsing error at startup.
     */
    ...
    /**
     * Counts current waitlisted registrations for an event.
     * Used to assign the next sequential waitlistPosition.
     */
